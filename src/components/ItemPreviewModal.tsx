...
import { ReadingTooltip, kanaToRomaji } from "@/components/ReadingBadges";
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 justify-center">
            {item.readingsOn.length > 0 && (
              <div className="flex items-center gap-2 bg-pink-50 px-3 py-1.5 rounded-full">
                <ReadingTooltip type="onyomi" kanji={item.character} readings={item.readingsOn}>
                  <span className="text-xs text-pink-600 font-medium">On'yomi</span>
                </ReadingTooltip>
                <span className="font-japanese text-sm sm:text-base text-stone-700">
                  {item.readingsOn.map(r => `${r} (${kanaToRomaji(r)})`).join("、")}
                </span>
              </div>
            )}
            {item.readingsKun.length > 0 && (
              <div className="flex items-center gap-2 bg-teal-50 px-3 py-1.5 rounded-full">
                <ReadingTooltip type="kunyomi" kanji={item.character} readings={item.readingsKun}>
                  <span className="text-xs text-teal-600 font-medium">Kun'yomi</span>
                </ReadingTooltip>
                <span className="font-japanese text-sm sm:text-base text-stone-700">
                  {item.readingsKun.map(r => `${r} (${kanaToRomaji(r)})`).join("、")}
                </span>
              </div>
            )}
          </div>
